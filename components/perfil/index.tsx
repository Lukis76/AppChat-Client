...
export const Profile: FC = () => {
  ...
  return (
    <>
      <button className="fixed w-28 h-28 top-0 left-0 bg-red-900 " onClick={() => setOpen((state) => !state)}></button>
      <section
      ...
      >
        <div className="flex flex-row justify-between items-center w-full" onClick={() => setOpen(false)}>
          <Image
            ...
            alt="logo app"
          />
          <button className="flex justify-center items-center p-2 m-2 bg-red-600 rounded-md">X</button>
        </div>
        <div>
          <Image
